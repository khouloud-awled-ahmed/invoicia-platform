...
export function SigningExperienceDialog({
  ...
  // Current signer (should be determined by authentication in real app)
  const currentSigner = envelope.signers.find((s: any) => s.status === 'in_progress' || s.status === 'pending' || s.status === 'WAITING' || s.status === 'SENT') || envelope.signers[0];

  // Signature fields from envelope
  const signatureFields = envelope.fields || [];

  const allRequiredFieldsFilled = () => { return true; };
  ...
  const handleSign = async () => {
    ...
    try {
      const signatureData = {
        fieldValues: [
          ...Object.entries(textFieldValues).map(([fieldId, value]) => ({ fieldId, value })),
          ...(typedSignature ? [{ fieldId: (envelope.fields?.find(f => f.type === 'SIGNATURE' || f.type === 'signature') || envelope.fields?.[0])?.id || 'signature', value: typedSignature, signatureData: typedSignature }] : []),
          ...(initialsValue ? [{ fieldId: 'initials', value: initialsValue }] : []),
        ],
      };
          ...
          <div className="border-b p-6 bg-gradient-to-r from-blue-50 to-blue-100">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mb-3">{envelope.documentName}</p>
                      ...
                      <>
                        ...
                        <div className="mt-12 p-6 border-2 border-dashed rounded-lg"
                          ...
                          <div className="mb-3 flex items-center justify-between">
                            <span className="font-semibold" style={{ color: currentSigner.color }}>
                              <PenTool className="w-4 h-4 inline mr-2" />
                              Signature requise *
                            </span>
                            {typedSignature && <CheckCircle2 className="w-5 h-5 text-green-600" />}
                          </div>
                          <div className="bg-white border-2 rounded h-32 flex items-center justify-center mb-3">
                            <div className={lex gap-2 mb-2}>
                            <button onClick={() => setSignatureType('draw')} className={signatureType === 'draw' ? 'px-3 py-1 text-xs rounded bg-blue-600 text-white' : 'px-3 py-1 text-xs rounded bg-gray-100'}>Dessiner</button>
                            <button onClick={() => setSignatureType('type')} className={signatureType === 'draw' ? 'px-3 py-1 text-xs rounded bg-blue-600 text-white' : 'px-3 py-1 text-xs rounded bg-gray-100'}>Taper</button>
                          </div>
                          {signatureType === 'draw' ? (
                            <div>
                              <canvas ref={canvasRef} width={380} height={120} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} className={order-2 border-gray-300 rounded cursor-crosshair w-full} style={{ background: 'white' }} />
                              <button onClick={clearCanvas} className={mt-1 text-xs text-red-500}>Effacer</button>
                            </div>
                          ) : (
                            <Input placeholder={Tapez votre nom complet...} value={typedSignature} onChange={(e) => setTypedSignature(e.target.value)} className={	ext-lg} />
                          )}
                          {typedSignature ? (
                              <p className="text-4xl font-signature italic" style={{ color: currentSigner.color }}>
                                {typedSignature}
                              </p>
                            ) : (
                              <p className="text-gray-400 text-sm">Votre signature apparaîtra ici</p>
                            )}
                          </div>
                          <Input
                      ...
                      <>
                        ...
                        <div className="mt-12 p-6 border-2 border-dashed rounded-lg"
                          ...
                          <div className="mb-3 flex items-center justify-between">
                            ...
                            {initialsValue && <CheckCircle2 className="w-5 h-5 text-green-600" />}
                          </div>
                          <div className="bg-white border-2 rounded h-20 flex items-center justify-center mb-3">
                            {initialsValue ? (
                              <p className="text-2xl font-semibold" style={{ color: currentSigner.color }}>
                                {initialsValue}
                              </p>
                            ) : (
                              <p className="text-gray-400 text-sm">Vos initiales apparaîtront ici</p>
                            )}
                          </div>
                          <Input
                        ...
                        {/* Text Field */}
                        <div className="mt-6 p-4 border-2 rounded-lg" style={{ borderColor: currentSigner.color }}>
                          <label className="font-semibold block mb-2" style={{ color: currentSigner.color }}>
                            Votre fonction (optionnel)
                          </label>
                          <Input
                            placeholder="Ex: Directeur Général, Responsable Achats..."
                            value={textFieldValues["text1"] || ""}
                            onChange={(e) => setTextFieldValues({ ...textFieldValues, text1: e.target.value })}
                          />
                        </div>
                      </>
                    )}
                      ...
                      <>
                        ...
                        <div className="mt-6 p-4 border-2 border-dashed rounded-lg"
                          ...
                          <div className="bg-white border-2 rounded h-16 flex items-center justify-center">
                            <p className="text-xl font-semibold" style={{ color: currentSigner.color }}>
                              {initialsValue || "—"}
                            </p>
                          </div>
                        </div>
                          ...
                          <div className="flex items-start gap-3">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) => 
                                setCheckboxValues({ ...checkboxValues, check1: checked as boolean })
                              }
                            />
                            <label htmlFor="check1" className="text-sm cursor-pointer">
                              <strong>J'accepte les termes et conditions *</strong>
                              <p className="text-muted-foreground mt-1">
                                Je certifie avoir lu et accepté l'intégralité du présent document.
                              </p>
                            </label>
                          </div>
                        </div>
            ...
            <div className="w-80 border-l bg-gray-50 p-6 overflow-y-auto">
              ...
              <div className="space-y-3 mb-6">
                {signatureFields.map(field => {
                  ...
                  if (field.type === "signature") isCompleted = typedSignature.length > 0;
                  if (field.type === "initials") isCompleted = initialsValue.length > 0;
                  if (field.type === "date") isCompleted = true;
                  if (field.type === "text") isCompleted = !field.required || textFieldValues[field.id]?.length > 0;
                  if (field.type === "checkbox") isCompleted = checkboxValues[field.id];
