...
1. **Option recommandée :** supprimer ou vider la base MongoDB utilisée par l’app (nom défini dans la config (**`INVOCIA-TN`**), ex. `mongosh` → `use INVOCIA-TN` → `db.dropDatabase()`), puis relancer :
   ```bash
  cd backend
  npm run seed
  ```
...
- **Marché :** **Invocia Tunisie** — devise TND, Matricule Fiscal (MF), TVA 19/13/7 %, timbre fiscal, El Fatoora (QR sur factures). **Base :** **`INVOCIA-TN`** (`backend/.env`).
...
- **Tests rapides :** Super Admin `admin@invocia.io` / `admin123` ; Client `client@test.com` / `client123` (tenant « Entreprise Test SAS », MF `1234567/A/B/M/000`).
