...
let AutomationService = class AutomationService {
    constructor(invoiceModel, projectModel) {
        this.invoiceModel = invoiceModel;
        this.projectModel = projectModel;
    }
    async getInvoiceableEntries(tenantId, filters) {
        return { entries: [], total: 0 };
    }
    async generateInvoices(tenantId, options) {
        return { message: 'Automatic invoice generation not yet implemented', invoices: [] };
    }
    async generateFromCRA(tenantId, options) {
